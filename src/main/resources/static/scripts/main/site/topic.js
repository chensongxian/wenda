...
$(function () {
    ...
    function fInitialize() {
        ...
        that.offset = 0;
        that.limit = 10;
        that.listHasNext=new Array();
        that.listHasNext[1] = true;

        that.renderMore(function () {
            // 没有数据隐藏加载更多按钮
            !that.listHasNext[that.topicId] && $('.zu-button-more').html("没有数据");;
        });
        ...
        $(".tag-cloud a").click(function () {
            $(".tag-cloud a").eq($(this).index()).addClass("navdown").siblings().removeClass("navdown");
            that.offset=0;
            that.topicId=$(this).index()+1;
            that.listHasNext[that.topicId] = true;
            that.listEl.empty();
            that.renderMore(function () {
                // 没有数据隐藏加载更多按钮
                !that.listHasNext[that.topicId] && $('.zu-button-more').html("没有数据");;
