(function (window, undefined) {
    ...
    $.extend(Business, {
        ...
        followQuestionByLink: fFollowQuestionByLink,
    });
    ...
    function fFollowQuestionByLink() {
        var that = this;

        $(document).on('click', '.follow-link', function (oEvent) {
            var oEl = $(oEvent.currentTarget);
            var sId = $.trim(oEl.attr('data-id'));
            if (!sId) {
                return;
            ...
            // alert(sId);
            oEl.attr('data-limit', '1');
            var bFollow=$('.z-icon-follow').attr('data-status')==='1';
            Action[bFollow ? 'unFollowQuestion' : 'followQuestion']({
                questionId: sId,
                call: function (oResult) {
                    // 修改标记位
                    $('.z-icon-follow').attr('data-status', bFollow ? '0' : '1');
                    // 文字
                    oEl.html(bFollow ? '<i class="z-icon-follow" data-status="1"></i>关注问题' : '<i class="z-icon-follow" data-status="1"></i>取消关注');
